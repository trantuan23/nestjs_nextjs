...
const ModalChangePassword = (props: any) => {
  ...
  const onFinishStep1 = async (values: any) => {
    ...
    if (password !== confirmPassword) {
      notification.error({
        ...
        description: "Password and changePassword invaild !",
      });
      return;
    }
    const res = await sendRequest<IBackendRes<any>>({
      url: `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/auth/change-passowrd`,
      method: "POST",
      body: {
        code,
        password,
  ...
  const resetModal = () =>{
    setIsModalOpen(false),setCurrent(0),setUserEmail("")
    form.resetFields()
  }
  return (
    <>
